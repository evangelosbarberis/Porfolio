...
* List.c
* Takes in a file and outputs file with all words in lexiographical order. 
* User enters file name and output file name, and program will output the alphabetically sorted words in the file, user entered output file.
* Also sends functions (preconditioned) to Lex.c
***/ 
...
bool equals(List A, List B) {
    if (A == NULL || B == NULL) {
        fprintf(stderr, "Error: equals() = NULL\n");
        exit(1);
    }
    ...
    // set up an iterator for each list
    Node i = A->front;
    Node j = B->front;
    // iterate through each list, checking if each element is equal
    while (i != NULL) {
        if (i->data != j->data) {
            return false;
        }
...
//creates a new list anc copies elements from a list
List copyList(List L){
    if(L == NULL){
        fprintf(stderr, "Error:  copyList() = Null\n");
        exit(1);
    }
    // new list here
    List newL = newList();
    Node N = L->front;
    while(N != NULL){ // traverse amd appends each element to
        append(newL, N->data);
        N = N->next;
    }
    return newL;
